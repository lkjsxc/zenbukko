...
import type { OcrTask } from './geminiOcrPlan.js';
import { DEFAULT_PROMPT, waitForFile } from './geminiOcrFlex.js';
...
export async function batchOcrPdfs(params: {
  ai: GoogleGenAI;
  tasks: OcrTask[];
  model: string;
  timeoutMs: number;
}): Promise<BatchOcrOutput> {
  const uploads = [];
